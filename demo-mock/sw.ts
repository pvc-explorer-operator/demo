...
self.addEventListener('fetch', (ev: any) => {
  ...
  if (ev.request.mode === 'navigate' && url.origin === self.location.origin) {
    ev.respondWith(caches.match(SHELL_URL).then(cached => cached ?? fetch(ev.request)))
    return
  }
  if (!url.pathname.startsWith('/api/')) return
  ev.respondWith(handleRequest(ev.request).then((res: Response | null) => res ?? fetch(ev.request)))
})
